...
abstract contract VaultHealerGate is VaultHealerBase {
    ...
    // Want tokens moved from user -> this -> Strat (compounding)
    function stratDeposit(uint256 _vid, uint256 _wantAmt) external whenNotPaused(_vid)  {
    ...
    function _withdraw(uint256 _vid, uint256 _wantAmt, address _to) private {
        ...
        //check that user actually has shares in this vid
        uint256 userUnboostedWant = balanceOf(_to, _vid) * vault.strat.wantLockedTotal() / totalSupply(_vid);
        uint256 userBoostedWant;
        if (address(boostPool) != address(0)) {
            userBoostedWant = boostPool.userStakedAmount(_to) * vault.strat.wantLockedTotal() / totalSupply(_vid);
        ...
        if (_wantAmt > userUnboostedWant && userBoostedWant > 0) { //&&boostPool exists! check that it's not a zero address?
            boostPool.withdraw((_wantAmt-userUnboostedWant)*totalSupply(_vid) / vault.strat.wantLockedTotal(), _to);
            }
