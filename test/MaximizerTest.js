...
describe(`Testing ${STRATEGY_CONTRACT_TYPE} contract with the following variables:
    ...
    before(async () => {
        ...
        if (TOKEN0ADDRESS == ethers.utils.getAddress(WMATIC) ){
            wmatic_token = await ethers.getContractAt(IWETH_abi, TOKEN0ADDRESS); 
            await wmatic_token.connect(user4).deposit({ value: ethers.utils.parseEther("4500") });
        } else {
            await uniswapRouter.connect(user4).swapExactETHForTokens(0, [WMATIC, TOKEN0ADDRESS], user4.address, Date.now() + 900, { value: ethers.utils.parseEther("4500") })
        }
        if (TOKEN1ADDRESS == ethers.utils.getAddress(WMATIC)) {
            wmatic_token = await ethers.getContractAt(IWETH_abi, TOKEN1ADDRESS); 
            await wmatic_token.connect(user4).deposit({ value: ethers.utils.parseEther("4500") });
        } else {
            await uniswapRouter.connect(user4).swapExactETHForTokens(0, [WMATIC, TOKEN1ADDRESS], user4.address, Date.now() + 900, { value: ethers.utils.parseEther("4500") })
        }
    ...
    `, () => {
        ...
        it('Should zap token0 into the vault (convert to underlying, add liquidity, and stake)', async () => {
            ...
            console.log(vaultSharesTotalBeforeFirstZap);
        ...
        it('Should withdraw remaining balance back to owner, minus withdrawal fee (0.1%)', async () => {
            ...
            const UsersStakedTokensBeforeFinalWithdrawal = await vaultHealer.stakedWantTokens(poolLength-1, user4.address);
            ...
            UsersStakedTokensAfterFinalWithdrawal = await vaultHealer.stakedWantTokens(poolLength-1, user4.address);
        ...
        it('Zap out should convert LP tokens back to underlying tokens, leaving a zero balance of LP tokens', async () => {
            const LPtokenBalanceBeforeZapOut = await LPtoken.balanceOf(user4.address);
