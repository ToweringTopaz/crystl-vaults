...
library CronaPriceGetter {
    ...
    bytes32 public constant INITCODEHASH = hex'511f0f358fe530cda0859ec20becf391718fdf5a329be02f4c95361f3d6a42d8'; // for pairs created by CronaFactory
    ...
    function getRawLPPrice(address lp, uint croPrice, uint ethPrice) internal view returns (uint) {
        ...
        try ICronaPair(lp).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {
            ...
            uint totalValue = normalize(getRawPrice(token0, croPrice, ethPrice), token0, DECIMALS) * reserve0 
                + normalize(getRawPrice(token1, croPrice, ethPrice), token1, DECIMALS) * reserve1;
    ...
    function getRawPrice(address token, uint croPrice, uint ethPrice) internal view returns (uint) {
        uint pegPrice = pegTokenPrice(token, croPrice, ethPrice);
        if (pegPrice != 0) return pegPrice;
